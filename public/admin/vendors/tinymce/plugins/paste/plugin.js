...
(function () {
    ...
    var pasteImage = function (editor, imageItem) {
        ...
        if (isValidDataUriImage(editor, img)) {
            ...
            if (!existingBlobInfo) {
                ...
                var name_1 = useFileName
                    ? extractFilename(editor, file.name)
                    : id;
        ...
        return global$3.all(
            map(items, function (item) {
                return new global$3(function (resolve) {
                    var blob = isDataTransferItem(item)
                        ? item.getAsFile()
                        : item;
    ...
    var getHtml = function (editor) {
        ...
        var dirtyWrappers = editor.dom.select(
            "div[id=mcepastebin]",
            pasteBinElm
        );
        ...
        return function (evt) {
            if (hasSelectedContent(editor)) {
                setClipboardData(
                    ...
                    function () {
                        if (
                            global$1.browser.isChrome() ||
                            global$1.browser.isFirefox()
                        ) {
        ...
        global.add("paste", function (editor) {
            if (hasProPlugin(editor) === false) {
                ...
                var pasteFormat = Cell(
                    isPasteAsTextEnabled(editor) ? "text" : "html"
                );
