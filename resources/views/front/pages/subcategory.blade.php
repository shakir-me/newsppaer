...
<input type="hidden" value="{{$subcategory->id}}" id="subcategory_id" />
           <ol class="breadcrumb details-page-breadcrumb">
             ...
             <li class="active"><a href=""> {{$subcategory->name}}</a></li>
               ...
               <div class="row custom-row content_list" id="grid_view_all{{$subcategory->id}}">
              ...
              <div class="row"  id="show_more_main343558">
                <div class="col-sm-12 text-center">
                  <div class="btn btn-more-details hvr-bounce-to-right">
                    <span id="ajax-loadmore-post{{$subcategory->id}}" class="show_more" title="Load more posts">আরও খবর</span>
...
$( document ).ready(function() {
 var subcategory_id = $('#subcategory_id').val()
            function loadmoreProduct(page) {
                $.ajax({
                        ...
                        beforeSend: function(response) {
                            $('#ajax-loadmore-post'+subcategory_id).hide();
                            $('#loading_post').show();
                        }
                    })
                    .done(function(data) {
                      $('#loading_post').hide();
                      $('#ajax-loadmore-post'+subcategory_id).show();
                      
                        if (data.grid_view_subcategory == "") {
                        
                           
                            $('#ajax-loadmore-post'+subcategory_id).html('No More Post Found');

                            $('#ajax-loadmore-post'+subcategory_id).unbind("click");
                            return;
                        }
                       
                        $('.ajax-loadmore-post'+subcategory_id).hide();
                        $('#grid_view_all'+subcategory_id).append(data.grid_view_subcategory);
            ...
            var page = 1;
            $("#ajax-loadmore-post"+subcategory_id).on("click",function() {
              page++;
